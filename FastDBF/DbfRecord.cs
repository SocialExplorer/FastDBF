...
{ 
  ...
  { 
    ...
    //Ascii Encoder
    private static readonly Encoding ASCIIEncoder = Encoding.ASCII;
      ...
      { 
        ...
        { 
          ...
          { 
            ...
            Buffer.BlockCopy(mEmptyRecord, ocol.DataAddress, mData, ocol.DataAddress, ocol.Length);
            ASCIIEncoder.GetBytes(value, 0, value.Length > ocol.Length ? ocol.Length : value.Length, mData, ocol.DataAddress);
          ...
          { 
            ...
            { 
              ...
              ASCIIEncoder.GetBytes(value, 0, nNumLen, mData, (ocol.DataAddress + ocol.Length - nNumLen));
            ...
            { 
              ...
              { 
                ...
                ASCIIEncoder.GetBytes(cDec, 0, nLen, mData, (ocol.DataAddress + ocol.Length - ocol.DecimalCount));
              }
              
              //set integer part, CAREFUL not to overflow buffer! (truncate instead)
              //-----------------------------------------------------------------------
              int nNumLen = cNum.Length > ocol.Length - ocol.DecimalCount - 1 ? (ocol.Length - ocol.DecimalCount - 1) : cNum.Length;
              ASCIIEncoder.GetBytes(cNum, 0, nNumLen, mData, ocol.DataAddress + ocol.Length - ocol.DecimalCount - nNumLen - 1);
          ...
          { 
            ...
            throw new Exception("Unrecognized data type: " + ocolType.ToString());
    ...
    { 
      ...
      { 
        ...
        //-------------------------------------------------------------
        ASCIIEncoder.GetBytes(value.ToString("yyyyMMdd"), 0, ocol.Length, mData, ocol.DataAddress);
    ...
    { 
      return new string(ASCIIEncoder.GetChars(mData));
    }
    ...
    { 
      if(obr.Read(mData, 0, mData.Length) < mData.Length)
        return false;
