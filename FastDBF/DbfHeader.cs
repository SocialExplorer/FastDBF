	...
	public class DbfHeader : ICloneable
	{
		...
		protected internal byte[] EmptyDataRecord
		{
			get
			{
				...
				if (mEmptyRecord == null)
				{
					//initialize array for clearing data quickly
					mEmptyRecord = Encoding.ASCII.GetBytes("".PadLeft((int)mRecordLength, ' ').ToCharArray());
		...
		public void Write(BinaryWriter writer)
		{
			...
			for (int i = 0; i < mFields.Count; i++)
			{
				char[] cname = mFields[i].Name.PadRight(11, (char)0).ToCharArray();
				writer.Write(cname);

				// write the field type
				writer.Write((char)mFields[i].ColumnTypeChar);
