...
namespace TestDbfLib
{
  ...
  class Program
  {
    ...
    static void TestWriteNewDbf()
    {	
			
      //create a simple DBF file and output to args[0]
      DbfFile odbf = new DbfFile();

      odbf.Open("P:\\Development\\Library\\CS\\FastDBF\\DBFSamples\\TestNew2.dbf", FileMode.Create);
      ...
      orec[0] = "Stefanie Singer longer than fits in the DBF record!";
      orec[1] = "0.1";
      orec[2] = ".12";
      orec[3] = ".1";
      orec[4] = "";
      ...
      odbf.Open("P:\\Development\\Library\\CS\\DbfLib\\DBFSamples\\TestNew2.dbf", FileMode.Open);
    ...
    static void TestReadFile()
    {
      ...
      odbf.Open("P:\\Development\\Library\\CS\\DbfLib\\DBFSamples\\TestNew2.dbf", FileMode.Open); //State_vars1.dbf  county_vars1.dbf
      ...
      FileStream ofs = new FileStream("P:\\Development\\Library\\CS\\DbfLib\\DBFSamples\\filesource.txt", FileMode.Create);
      StreamWriter osw = new StreamWriter(ofs, Encoding.Default);
    ...
    static void TestReadHttpConn()
    {
      //Open a web file...
      //-------------------------------------
      DbfFile oWebDBF = new DbfFile();
      WebClient oWebFile = new WebClient();

      oWebDBF.Open(oWebFile.OpenRead("http://private.socialexplorer.com/State_vars1.dbf"));

      //read and print records to screen...
      DbfRecord orecWeb = new DbfRecord(oWebDBF.Header);

      FileStream ofs2 = new FileStream("P:\\Development\\Library\\CS\\FastDBF\\DBFSamples\\Webfile.txt", FileMode.Create);
      StreamWriter osw2 = new StreamWriter(ofs2);
      ...
      {
        osw2.WriteLine("index: " + orecWeb.RecordIndex + ": " + orecWeb.ToString());
      }
